...
const TrainResult = () => {
  ...
  useEffect(() => {
    const fetchTrains = async () => {
      ...
      setLoading(true);
   try {
  const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/trains/search?from=${from}&to=${to}`);
  ...
  useEffect(() => {
    const getFaresForDates = async () => {
      ...
      for (const date of dateStrings) {
       ...
       try {
  const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/fares/bulk`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
 ...
 try {
  const res = await fetch(
    `${import.meta.env.VITE_API_BASE_URL}/api/booking/train/${trainId}/booking-info?from=${from}&to=${to}&classType=${classType}&date=${dateStr}`
  );
  const data = await res.json();
